...
def _live_fallback(query: str, winkel: str) -> Optional[dict]:
    """Live API-zoektocht als de vector-index geen goede match heeft. Past
    dezelfde relevantie- en ruisfilters toe als de vector-match (via
    _kies_beste), zodat ook hier geen willekeurig goedkoop product wint."""
...
def node_vergelijk_prijzen(state: AgentState) -> AgentState:
    ...
    Gebruikt per item de eventueel VERFIJNDE zoekterm uit `zoek_queries`
    (gevuld door de feedbackloop). Bij de eerste ronde is dat gewoon de
    ...
    # Effectieve zoekterm per item: verfijnde versie indien aanwezig, anders de naam
    ...
    for item, query, ((ah_match, ah_bron), (jumbo_match, jumbo_bron)) in zip(items, queries, zoekresultaten):
        ...
        details.append({
            "naam": item.naam,          # sleutel voor de feedbackloop
            "query": query,             # zoekterm die deze ronde is gebruikt
            "label": item.label(),
        ...
        logger.debug(f"{item.label()}{zoek_str}: AH €{ah_prijs_tot:.2f} ({'✓' if ah_ok else '✗'}, {ah_bron}{ah_sim_str}) → {ah_titel}")
        logger.debug(f"{item.label()}{zoek_str}: Jumbo €{jumbo_prijs_tot:.2f} ({'✓' if jumbo_ok else '✗'}, {jumbo_bron}{jumbo_sim_str}) → {jumbo_titel}")
